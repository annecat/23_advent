...
fn main() {
    ...
    for line in lines {
        ...
        total += calc_possibilities(&sauna_left, 0, &numbers, 0, numbers[0], &memory);
...
fn calc_possibilities(sauna_left: &str, index_left:usize, numbers: &Vec<i32>, mut index_right:usize, mut current_num:i32, mut memory: &HashMap<(usize, usize, i32), usize> ) -> usize {
    ...
    match sauna_left.chars().nth(index_left) {
        Some('#') => {
            ...
            return calc_possibilities(sauna_left, index_left + 1, numbers, index_right, current_num - 1, memory);
        },
        Some('.') => {
            ...
            return calc_possibilities(sauna_left, index_left + 1, numbers, index_right, current_num, memory);
        },
        Some('?') => {
            //We combine what was done before
            //either the ? is a # 
            let tmp = if current_num <= 0 { 0 } else { calc_possibilities(sauna_left, index_left + 1, numbers, index_right, current_num - 1, memory) };
